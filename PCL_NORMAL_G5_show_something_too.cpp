...
main (int argc, char** argv)
{
    ...
    // Set parameters
    mls.setInputCloud(boost::make_shared <const pcl::PointCloud<pcl::PointXYZ> > (*cloud));
...
//    Normals=PointWithNormals;
    ...
    pcl::PrincipalCurvaturesEstimation<pcl::PointXYZ, pcl::PointNormal, pcl::PrincipalCurvatures> principalCurvaturesEstimation;

// Provide the original point cloud (without normals)
    principalCurvaturesEstimation.setInputCloud (cloud);

// Provide the point cloud with normals
    principalCurvaturesEstimation.setInputNormals(PointWithNormals->makeShared());
...
//    return 0;
    ...
    pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointNormal> color_handler (PointWithNormals->makeShared(), "curvature");
    if (!color_handler.isCapable ()) {
        PCL_WARN ("Cannot create curvature color handler!");
    }
    viewer.addPointCloud(PointWithNormals->makeShared(),color_handler,"myCLooud");
